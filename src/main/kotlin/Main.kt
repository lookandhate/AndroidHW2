...
fun Shop.getCustomersWithMoreUndeliveredOrdersThanDelivered(): Set<Customer> =
    customers.filter { it.orders.count { order -> order.isDelivered } < it.orders.count { order -> !order.isDelivered } }
        .toSet()

// Вернуть наиболее дорогой продукт из всех доставленных
fun Customer.getMostExpensiveDeliveredProduct(): Product? =
    orders.filter { order -> order.isDelivered }.flatMap { it.products }.maxByOrNull { it.price }
